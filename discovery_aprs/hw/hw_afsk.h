 ...
 */
#define AFSK_ADC_INIT(ch, ctx) do {  } while (0)

#define AFSK_STROBE_ON()/*							\
		do {							\
			stm32_gpioPinWrite(GPIOC_BASE, BV(8), 1);	\
		} while (0)
*/
#define AFSK_STROBE_OFF()/*							\
		do {							\
			stm32_gpioPinWrite(GPIOC_BASE, BV(8), 0);	\
		} while (0)
*/
#define AFSK_STROBE_INIT()	/*						\
		do {							\
			RCC->APB2ENR |= RCC_APB2_GPIOC;			\
			stm32_gpioPinConfig(GPIOC_BASE,		\
...
void DA_Start(void);
void DA_Stop(void);
#define AFSK_DAC_IRQ_START(ch)   do { DA_Start(); } while (0)

/**
 * Stop DAC convertions on channel \a ch.
 * \param ch DAC channel.
 */
#define AFSK_DAC_IRQ_STOP(ch)    do { DA_Stop(); } while (0)
